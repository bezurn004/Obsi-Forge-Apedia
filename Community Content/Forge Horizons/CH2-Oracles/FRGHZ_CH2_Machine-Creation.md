---
## Source
SourceMaterial: "Forge Horizons"
SourceAuthor: "Michael Elliott"
SourceLink: 

## Page
aliases:
  - "Machine - Creation (Forge Horizons)"
  - "Machine Creation - Environment (Forge Horizons Oracle)"
  - "Machine Creation - Scale (Forge Horizons Oracle)"
  - "Machine Creation - Ultra Scale (Forge Horizons Oracle)"
  - "Machine Creation - Basic Form (Forge Horizons Oracle)"
  - "Machine Creation - Propulsion (Forge Horizons Oracle)"
PageType: Oracle
PageCategory: Machines
PageOrder: 9

## Oracle
Oracle: Machine
OracleFocus: Creation
OracleDescriptor: "Start to build a machine character for your story"
---
# [[_Forge Horizons|Forge Horizons]] - [[_FRGHZ_CH2_Oracles|Oracles]]: Machine Creation

## Oracle: Environment
> [!mechanics]- | Roll > `dice: [[OT_FRGHZ_Machines#^Environment]]` | Guidance
> If you know where this machine is, you can skip this step, otherwiseroll for where the machine is found, or is designed to operate in. ^guidance-Environment

| dice: 1d100 | Result |
| :---: | --- |
| 1 - 20 | Space |
| 21 - 50 | Interior |
| 51 - 70 | Land |
| 71 - 80 | Liquid |
| 81 - 99 | Air |
| 99 - 100 | Roll twice |
^table-Environment

## Oracle: Scale
> [!mechanics]- | Roll > `dice: [[OT_FRGHZ_Machines#^Scale]]` | Guidance
> TEXT ^guidance-Scale

| dice: 1d100 | Result |
| :---: | --- |
| 1 - 10 | Nano (microscopic) |
| 11 - 20 | Minuscule (battery-sized) |
| 21 - 35 | Tiny (cell phone-sized) |
| 36 - 50 | Small (car engine-sized) |
| 51 - 70 | Medium (person-sized) |
| 71 - 90 | Large (vehicle-sized) |
| 91 - 95 | Huge (starship-sized) |
| 96 - 100 | ▶[[#Oracle Ultra Scale\|Ultra scale]] (roll below) |
^table-Scale

## Oracle: Ultra Scale
> [!mechanics]- | Roll > `dice: [[OT_FRGHZ_Machines#^Ultra-Scale]]` | Guidance
> TEXT ^guidance-Ultra-Scale

| dice: 1d100 | Result |
| :---: | --- |
| 1 - 89 | Titanic (skyscraper-sized) |
| 90 - 99 | Colossal (city-sized) |
| 100 | Vast (planet-sized) |
^table-Ultra-Scale

*Page 15*

## Oracle: Basic Machine Form
> [!mechanics]- | Roll > `dice: [[OT_FRGHZ_Machines#^Form]]` | Guidance
> TEXT ^guidance-Form

| dice: 1d100 | Result |
| :---: | --- |
| 1 - 5 | Cylinder |
| 6 - 15 | Egg |
| 16 - 35 | Humanoid |
| 36 - 55 | Insectoid |
| 56 - 60 | Pyramid |
| 61 - 70 | Rectangle |
| 71 - 85 | Sphere |
| 86 - 100 | Square |
^table-Form

## Oracle: Machine Propulsion
> [!mechanics]- | Roll > `dice: [[OT_FRGHZ_Machines#^Propulsion]]` | Guidance
> TEXT ^guidance-Propulsion

| dice: 1d100 | Result |
| :---: | --- |
| 1 - 5 | Anti-gravity |
| 6 - 15 | Biped |
| 16 - 35 | Quadruped |
| 36 - 50 | Rotors |
| 51 - 65 | Treads |
| 66 - 85 | Tripod |
| 86 - 95 | Wheels |
| 96 - 100 | Wings/fins |
^table-Propulsion

*Page 16*

## Tags
| Section Page | Tags | Next Page |
| :--- | :---: | ---: |
| **[[FRGHZ_CH2_Ecumenopolis-Details\|Ecumenopolis - Details (Forge Horizons)]]** | #ForgeHorizons/Oracles | **[[FRGHZ_CH2_Machine-Approach\|Machine - Approach (Forge Horizons)]]** |

<font size=-2>Forge Horizons is created by Michael Elliott and licensed for use under the Creative Commons Attribution 4.0 International License (CC-BY). Work is permitted by author for use in this collection of materials.</font>